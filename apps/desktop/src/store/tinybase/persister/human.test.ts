...
import { createHumanPersister, jsonToContent, storeToJson } from "./human";
...
describe("humanPersister roundtrip", () => {
  ...
  test("store -> json -> store preserves all data", () => {
    ...
    store.setTable("humans", originalHumans);
    const json = storeToJson(store);
    const [tables] = jsonToContent(json);

    expect(tables).toEqual({ humans: originalHumans });
  });
