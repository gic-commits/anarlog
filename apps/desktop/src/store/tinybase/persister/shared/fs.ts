...
/** @deprecated Use createFileListener with { mode: "simple" } config instead */
...
/** @deprecated Use createFileListener with { mode: "entity" } config instead */
export function createEntityListener(
  pathMatcher: (path: string) => boolean,
  entityParser: (path: string) => string | null,
  debounceMs = DEFAULT_DEBOUNCE_MS,
): {
  addListener: (listener: (entity: EntityInfo) => void) => NotifyListenerHandle;
  delListener: (handle: NotifyListenerHandle) => void;
} {
  return {
    addListener: (listener: (entity: EntityInfo) => void) => {
      const handle: NotifyListenerHandle = {
        unlisten: null,
        interval: null,
        debounceTimeout: null,
      ...
      (async () => {
        try {
          ...
          console.error(
            "[EntityListener] Failed to setup notify listener:",
            error,
          );
        }
      })();
