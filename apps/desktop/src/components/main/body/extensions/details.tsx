  ...
  if (!selectedExtensionId) {
    return (
      <div className="h-full flex items-center justify-center">
        <p className="text-sm text-neutral-500">Select an extension to view details</p>
      </div>
    );
  }

  return <ExtensionDetails key={selectedExtensionId} id={selectedExtensionId} />;
}
      ...
      <div className="px-6 py-4 border-b border-neutral-200">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-neutral-500 mt-1">Version {extension.version}</p>
          </div>
        ...
        <div className="space-y-6">
          {extension.description && (
            <div>
              ...
              <p className="text-sm text-neutral-600">{extension.description}</p>
            </div>
          )}
          ...
          <div>
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                ...
                <span className="font-mono text-neutral-900">{extension.id}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-neutral-500">API Version</span>
                <span className="font-mono text-neutral-900">{extension.api_version}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-neutral-500">Panels</span>
                <span className="text-neutral-900">{extension.panels.length}</span>
              </div>
            </div>
          </div>
          ...
          {extension.panels.length > 0 && (
            <div>
              <h3 className="text-sm font-medium text-neutral-900 mb-2">Panels</h3>
                ...
                {extension.panels.map((panel) => (
                  ...
                  >
                    <div>
                      ...
                      <p className="text-xs text-neutral-500 font-mono">{panel.id}</p>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div>
            ...
            >
              <FolderOpen size={14} />
              <span className="font-mono text-xs truncate max-w-[300px]">{extension.path}</span>
