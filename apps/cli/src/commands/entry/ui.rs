...
pub fn draw(frame: &mut Frame, app: &mut EntryApp) {
    ...
    let [logo_area, title_area, tip_area, _gap, input_area, hint_area] = Layout::vertical([
...
fn draw_tip(frame: &mut Frame, area: Rect, app: &EntryApp, theme: &Theme) {
    frame.render_widget(
        Paragraph::new(Line::from(vec![
            Span::styled("Tip: ", Style::new().fg(Color::Cyan)),
