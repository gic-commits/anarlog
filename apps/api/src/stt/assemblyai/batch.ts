...
const createTranscript = async (audioUrl: string, params: BatchParams): Promise<string> => {
  const languageCode =
    params.languages && params.languages.length === 1 ? params.languages[0] : undefined;
  const languageDetection =
    !params.languages || params.languages.length === 0 || params.languages.length > 1;
...
const pollTranscript = async (transcriptId: string): Promise<AssemblyAITranscriptResponse> => {
  for (let attempt = 0; attempt < MAX_POLL_ATTEMPTS; attempt++) {
    ...
    if (!response.ok) {
      const errorText = await response.text();
      throw new Error(`AssemblyAI poll failed: ${response.status} - ${errorText}`);
    }
...
const convertToResponse = (result: AssemblyAITranscriptResponse): BatchResponse => {
  const words: BatchWord[] = (result.words ?? []).map((w) => {
    const speaker = w.speaker ? parseInt(w.speaker.replace(/\D/g, ""), 10) : undefined;
