...
async function fetchGitHub(url: string): Promise<Response> {
  return fetchWithCache(url, { headers: getGitHubHeaders() }, { ttl: CACHE_TTL, durable: true });
}

export const getGitHubStats = createServerFn({ method: "GET" }).handler(async () => {
  try {
    const response = await fetchGitHub(`https://api.github.com/repos/${GITHUB_ORG_REPO}`);
...
export const getStargazers = createServerFn({ method: "GET" }).handler(async () => {
  try {
    const repoResponse = await fetchGitHub(`https://api.github.com/repos/${GITHUB_ORG_REPO}`);
