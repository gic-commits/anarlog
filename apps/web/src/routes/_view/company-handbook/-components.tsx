...
function PageNavigation({ currentSlug }: { currentSlug: string }) {
  const { prev, next } = useMemo(() => {
    const orderedPages = handbookStructure.sections.flatMap((sectionId) => {
      const sectionTitle = handbookStructure.sectionTitles[sectionId];
      return allHandbooks
        .filter((doc) => doc.section === sectionTitle && !doc.isIndex)
