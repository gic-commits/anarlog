...
export const Route = createFileRoute("/_view/gallery/$type/$slug")({
  ...
  head: ({ loaderData }) => {
    ...
    const ogImageUrl = `https://hyprnote.com/og?type=gallery&title=${encodeURIComponent(item.title)}&category=${encodeURIComponent(item.category)}${item.description ? `&description=${encodeURIComponent(item.description)}` : ""}`;
