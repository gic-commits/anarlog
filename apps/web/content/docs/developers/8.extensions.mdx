...
title: "Extensions"
section: "Developers"
description: "Learn how to create and develop extensions for Char"
---

# Overview

Extensions allow you to add custom functionality to Char through panels that appear in the sidebar. Each extension can define one or more panels with custom React-based UIs that integrate seamlessly with the application.

Extensions consist of two main parts: a runtime script (`main.js`) that runs in a sandboxed Deno environment, and optional UI panels (`ui.tsx`) that render React components within the Char interface.
...
```json
{
  ...
  "version": "0.1.0",
  "api_version": "0.1",
  "description": "A custom extension for Char",
  "entry": "main.js",
  "panels": [
    {
...
- `api_version`: Char extension API version (currently `0.1`)
...
Panel UIs are React components that render within the Char interface. Create a `ui.tsx` file with a default export:
...
Extension UIs have access to the following globals provided by Char:

## React and UI

- `react` - React library
- `react-dom` - React DOM library
- `@hypr/ui` - Char UI component library (Button, ButtonGroup, Card, Checkbox, Popover)
- `@hypr/utils` - Char utility functions (cn, date-fns helpers, etc.)
...
Extensions can read state from the main Char application using TinyBase. The synchronization happens automatically via postMessage between the iframe and parent window.
...
Launch Char in development mode and navigate to your extension panel.
...
Then launch Char with `ONBOARDING=0 pnpm -F desktop tauri dev` and click on the profile area (shows "Unknown" by default) to expand the menu, then click "Hello World" to see the extension panel.
