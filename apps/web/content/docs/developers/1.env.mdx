...
These variables are used by the `transcribe` and `record` subcommands. See the [CLI Reference](/docs/cli) or run `char --help` for full usage.
...
PYANNOTE_API_KEY=""                    # optional STT provider (pyannoteAI, batch only)
