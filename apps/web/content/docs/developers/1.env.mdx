...
At least one STT provider API key is required for transcription to work. The Exa and Jina keys are required for the AI chat's web search and content retrieval features. The Nango and Stripe variables are required for calendar and subscription APIs. The GitHub and support database variables are required for the in-app support chat MCP server, which handles GitHub issue management and billing operations.
