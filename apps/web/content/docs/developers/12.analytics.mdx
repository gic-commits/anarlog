...
- Web auth callback calls `identify(userId, { email, plan, trial_end_date })` in `apps/web/src/routes/_view/callback/auth.tsx`.
...
| `upgrade_clicked` | `plan` (`"lite"` or `"pro"`) | `apps/desktop/src/settings/general/account.tsx` |
...
| Event | Properties | Notes |
|-------|------------|-------|
| `onboarding_step_viewed` | `step`, `platform` | Fired per step. macOS steps: `permissions` → `login` → `calendar` → `final`. Other platforms: `login` → `calendar` → `final`. |
| `user_signed_in` | none | Fires on sign-in. Desktop auth sync also updates `plan` and `trial_end_date` on sign-in, initial session restore, and token refresh. |
...
| `upgrade_clicked` | `plan` (`"lite"` or `"pro"`) | Desktop | User clicked an upgrade or switch button. This is intent, not completion. |

User properties for segmentation: `plan`, `trial_end_date`.

Gap: no explicit `subscription_started` or `payment_completed` event. Conversion from trial to paid is currently inferred from the `plan` user property after auth or billing refresh, or from Stripe data, not a PostHog event.
...
|----------|---------------|--------|
| `email` | `identify(..., { email })` (desktop and web) | `apps/desktop/src/auth/context.tsx`, `apps/web/src/routes/_view/callback/auth.tsx`, `apps/web/src/routes/_view/app/account.tsx` |
...
| `plan` | desktop `identify(...)` on sign-in, initial session restore, and token refresh; web `identify(...)` on auth callback and billing return; server `set_properties` on successful trial start | `apps/desktop/src/auth/context.tsx`, `apps/web/src/routes/_view/callback/auth.tsx`, `apps/web/src/routes/_view/app/account.tsx`, `crates/api-subscription/src/trial.rs` |
| `trial_end_date` | desktop `identify(...)` on sign-in, initial session restore, and token refresh; web `identify(...)` on auth callback and billing return; server `set_properties` on successful trial start (`UTC now + 14 days`) | `apps/desktop/src/auth/context.tsx`, `apps/web/src/routes/_view/callback/auth.tsx`, `apps/web/src/routes/_view/app/account.tsx`, `crates/api-subscription/src/trial.rs` |
