...
- `models/stt/` — downloaded speech-to-text model files (Cactus model archives)
...
- Cactus models (Whisper Small/Medium and Parakeet TDT 0.6B V3 in INT4/INT8 quantizations, with Apple NPU variants) — see [Local Models](/docs/developers/local-models#cactus-engine-beta)
...
- **Pro curated models:** Audio is proxied through the Char cloud proxy and forwarded to a curated STT provider. The proxy does not store your audio.
...
| pyannoteAI | [Privacy Policy](https://www.pyannote.ai/privacy) |
...
**Where it goes:**
- Proxied through the Char cloud proxy
...
The analytics client sends events to PostHog:
...
**What happens:**
- Standard HTTP download requests to S3 (Cactus model archives)
...
- Auth tokens to the Char cloud proxy when using Pro features
...
- [API server](https://github.com/fastrepl/char/tree/main/apps/api)
