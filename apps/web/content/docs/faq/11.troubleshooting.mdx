...
In release builds, Char also checks for updates in the background every 30 minutes and automatically downloads an update when one is available. If you have analytics enabled, Char may also send anonymous usage events to PostHog. Release builds also include Sentry for crash reporting.
