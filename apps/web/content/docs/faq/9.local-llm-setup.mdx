...
Some local models (e.g., Gemma) do not support tool calling, which Hyprnote uses for structured output during summarization. Hyprnote automatically detects this and falls back to plain text generation with JSON extraction, so summarization still works with these models. No configuration is needed.
