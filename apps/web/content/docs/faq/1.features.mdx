...
Yes. Charlie can be expanded into a full tab for a more spacious conversation view. You can also open a support chat tab from the profile menu in the sidebar by selecting **Report Bug** or **Suggest Feature**.
...
## Can I install the Char CLI from the desktop app?

Yes. Go to **Settings** > **Lab** > **Command-line tool** to install or manage the embedded CLI. The installer symlinks the bundled `char` binary so you can run CLI commands (transcribe, record, etc.) directly from your terminal. The panel shows the current install status and lets you reinstall or remove the symlink. This feature is currently macOS-only.

## Can I personalize how the AI chat responds?

Yes. Go to **Settings** > **Personalization** to adjust the chat assistant's tone, warmth, enthusiasm, use of headers/lists, and emoji. You can also provide freeform custom instructions. These preferences are injected into the chat system prompt so every response reflects your style.
...
Yes. Drag-and-drop or paste any file into the editor to attach it to the current session. Images (PNG, JPEG, GIF, WebP) render inline, while other files (PDFs, Word documents, spreadsheets, plain text, and more) are embedded as file attachment cards you can click to open in the system default app. Attachments are stored on disk alongside the session under `sessions/<id>/attachments/`, so they travel with your note like the rest of your data.
