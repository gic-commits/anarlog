...
1. **Char**: Complete local processing for compliance-sensitive organizations that can't use cloud tools
...
[Char](/) is the only truly local-first, open-source AI notepad where not a single byte of your data has to leave your device.

While most competitors process everything in the cloud, Char runs entirely on-device using custom AI models, which fundamentally changes what's possible from a compliance perspective.
...
<Image src="/api/images/blog/enterprise-ai-notetaking-tools/enterprise-2.webp" alt="Char pricing" />

Char offers all core features and unlimited AI summaries free forever. Consider upgrading to Pro ($8/month) for unlimited everything.
...
<Image src="/api/images/blog/enterprise-ai-notetaking-tools/enterprise-3.webp" alt="How does Char work" />

Char uses two types of models: speech-to-text (STT) and large language models (LLMs).

For transcription, Char utilizes the Whisper series from Hugging Face, which ranges from the lightweight Tiny and Base models to the V3 Large Turbo. Users choose based on their accuracy vs. speed requirements.
...
Char captures both microphone input and system audio output, allowing it to work universally across various meeting platforms without requiring bots. No internet required for core functionality.
...
Whether you need edge-only deployment for maximum IP protection, all-prem for complete infrastructure control, or prem-hybrid to balance compliance with powerful AI, Char's flexible architecture adapts to your requirements.
