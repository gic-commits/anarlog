...
sequenceDiagram
    ...
    participant RateLimiter as UserRateLimiter
    ...
    Note over Restate: Status: TRANSCRIBED
    Note over Restate: Status: LLM_RUNNING
    
    Restate->>LLM: POST /chat (transcript, userId)
    LLM-->>Restate: llmResult
    
    Note over Restate: Status: DONE
    
    Restate-->>Server: { status: DONE, transcript, llmResult }
