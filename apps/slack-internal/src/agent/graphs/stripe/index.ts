...
const executeStripeCodeTool = tool(
  async ({ code }: { code: string }) => {
    const result = await executeCode(code);
    return JSON.stringify(result);
  },
  {
    name: "executeStripeCode",
    description: "Execute TypeScript/JavaScript code to interact with Stripe",
    schema: z.object({
...
const tools = [executeStripeCodeTool];
const modelWithTools = model.bindTools(tools);

const callModel = task("callStripeModel", async (messages: BaseMessage[]) => {
  return modelWithTools.invoke(messages);
});

const callTool = task("callStripeTool", async (toolCall: ToolCall) => {
  ...
  const result = await executeStripeCodeTool.invoke(
...
export const stripeGraph = entrypoint(
  { name: "stripeGraph" },
