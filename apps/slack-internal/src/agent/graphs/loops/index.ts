...
const executeLoopsCodeTool = tool(
  async ({ code }: { code: string }) => {
    const result = await executeCode(code);
    return JSON.stringify(result);
  },
  {
    name: "executeLoopsCode",
    description: "Execute TypeScript/JavaScript code to interact with Loops.so",
    schema: z.object({
...
const tools = [executeLoopsCodeTool];
const modelWithTools = model.bindTools(tools);

const callModel = task("callLoopsModel", async (messages: BaseMessage[]) => {
  return modelWithTools.invoke(messages);
});

const callTool = task("callLoopsTool", async (toolCall: ToolCall) => {
  ...
  const result = await executeLoopsCodeTool.invoke(
...
export const loopsGraph = entrypoint(
  { name: "loopsGraph" },
