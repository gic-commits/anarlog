...
- **本地说话人分离（diarization）** —— 完全本地的流水线（pyannote
  分割 ONNX + 说话人嵌入 + HDBSCAN 密度聚类），本地标注"谁在什么
  时候说话"，音频不出设备。默认使用更快的 Cam++ 模型（比
  Wespeaker 快约 2 倍且分离效果更好）；80+ 分钟长会议也不会过度
  聚类成假说话人。
...
- **gap 自动重试** —— 当 provider 的词时间戳漂移造成内容空洞时，
  相关段会自动重新提交一次（若仍有空洞，则保留 Continue 入口供
  手动续传）。
- **麦克风输入设备选择** —— 可在录音前、甚至**录音过程中**切换
  输入设备（录音按钮或顶部 header 处），并带有录音计时器。
...
- **长转录性能** —— 数小时的转录采用虚拟化渲染（只挂载视口附近
  的段），渲染结果缓存，再次打开会议即刻显示。
