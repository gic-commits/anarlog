  ...
  return (
    <Popover>
      <PopoverTrigger>
        ...
        >
          <Users2Icon size={14} className="flex-shrink-0 text-neutral-700" />
          <span className="truncate text-neutral-700">{getButtonText()}</span>
          {count > 1 && !isVeryNarrow && !isNarrow && <span className="text-neutral-500">+ {count - 1}</span>}
        </div>
      </PopoverTrigger>
    ...
    >
      <div className="flex items-center gap-2.5 relative min-w-0">
        ...
        <div className="flex flex-col min-w-0 flex-1">
          ...
          {participant.job_title && <span className="text-xs text-neutral-500 truncate">{participant.job_title}</span>}
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 transition-colors flex-shrink-0">
        {participant.email && (
          <a
            ...
            className="text-neutral-500 transition-colors hover:text-neutral-700 p-1 rounded-full hover:bg-neutral-200"
          >
      ...
      <div className="flex flex-col gap-2">
        <div className="flex items-center w-full px-2 py-1.5 gap-2 rounded bg-neutral-50 border border-neutral-200">
          ...
          {value.trim() && (
            <button
              type="submit"
              className="text-neutral-700 hover:text-neutral-700 transition-colors flex-shrink-0"
            >
            ...
            {searchResults.length === 0 && (
              ...
              >
                <span className="flex items-center gap-1 font-medium text-neutral-700">
                  Create <span className="text-neutral-700">"{value.trim()}"</span>
                </span>
              </button>
            )}
          </div>
