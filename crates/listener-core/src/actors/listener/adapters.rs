...
> {
    ...
    tracing::info!(
        "[DEBUG] spawn_rx_task: adapter_kind={:?} base_url={} model={} provider={:?}",
        adapter_kind,
        args.base_url,
        args.model,
        args.provider,
    ...
    let is_dual = matches!(args.mode, crate::actors::ChannelMode::MicAndSpeaker)
        && !matches!(adapter_kind, AdapterKind::OpenAI);
...
> {
    ...
    tracing::info!(
        "[DEBUG] spawn_rx_task_single_with_adapter: building client base_url={} model={}",
        args.base_url,
        args.model,
    );
