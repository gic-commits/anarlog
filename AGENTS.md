...
## Aug 2 — 服务端两个问题确认修复（丢段 + whisper 字节敏感）✅
...
### ② whisper 字节敏感（1:05-1:30 段 whisper loop / 500）— 已报服务端并确认修复 ✅

**问题**：re-transcript 实测 1:05-1:30 段缺失。日志 metadata：`segments_total: 7, segments_stitched: 6, abandoned_segments: 3, gap_ms: 11261` → **group3（67.88-87.45s）被 abandoned**。

**根因（客户端复现 + 对照实验定位）**：
- 客户端 rodio 解码的 group3（626240B）单独 POST → **稳定 HTTP 500**（0.2s 立即失败）；拆半段后 200 但前半是 100+ 次重复"對"的 garbage。
- 同段 **ffmpeg 解码**（字节数相同）→ **完全正常**（"我覺得是有的就是...因為中國是一個非常大的國家..."）。
- 两解码器内容等价（能量包络相关 0.9947）但**逐字节不同**（~83% 字节不同）。
- rodio 版 whisper 陷入**重复输出 loop**（`avg_logprob=-0.078` vs 正常 `-0.50`）→ worker 内部异常 → 500。

**修复验证（Aug 2）**：服务端修复后用客户端 rodio 解码路径重新生成 group3 字节 POST：
- 之前稳定 500 → 现在 **HTTP 200 且转写正常**（"对对对,你知道中国是在发展的吗?我觉得是有的就是..."，62 words 覆盖到 19.44s，avg_logprob=-0.473 正常值非 loop）。
- **3/3 次稳定 200**；7 组全部 200 且内容完整无截断。
- 无需客户端改动。（注：验证期间服务端短暂重启导致连接 000，不影响结论）

**客户端侧候选防御（若再遇此类问题）**：检测响应中重复词比例 / `avg_logprob` 异常高 → 对该组换 ffmpeg 解码重试。
...
### 真机 live 录音测试（session 2495e5a7，4 人对话 146.9s）

**验证点**：progressive live 录音 diarization 端到端。

| 验证点 | 结果 |
|--------|------|
| diarization 引擎启动 | ✅ `diarization engine initialized for live stream` |
| 结果完成 | ✅ `progressive_batch session finished`，303 words，3 unique speakers [1,2,5]（实际 4，少 1 = #7 已知限制）|
| **丢段** | ❌ 3 个 gap_warnings（段间 10.8s/10.5s/19s，共 ~40s 空洞）→ 女声段疑似丢失 |
| **声音忽大忽小** | ❌ MicAndSpeaker 混音 + 丢帧叠加 |
| **无完成提醒** | ⚠️ 后端 `waitForLiveBatchResult completed` 但 UI 无提示（前端通知问题，待查）|

**根因定位**：#10 PCM 丢帧（`pipeline.rs` try_send 静默丢弃）+ #11 混音削波（`mix_sample_f32` clamp）。实测 VAD 平均 1.28ms（预算 120ms），丢帧为偶发尖峰（max 117ms）。**溯源：#11 由 `cb8913b7b`（Aug 1）引入**（此前 progressive 只发 AEC mic，无混音）。
...
| 12 | **live 停止后无完成提醒**（前端通知问题）| ✅ 已解决（Aug 9）| 根因：长 transcript 完成后界面刷新极慢（白屏），用户感知为"无完成提示"。**Aug 9 虚拟化（lazyload，words>1500 触发）+ 渲染缓存**大幅加快完成界面刷新后自然消除——完成事件正常发出，界面快速更新可见。无需额外通知链路 |
...
| 17 | **编译 warning 清理**（`d09623d7d` 引入）| 📝 待清理 | `plugins/transcription/src/listener/runtime.rs`：① `let mut diarization_engine`（218 行）不再需要 `mut`（已移入 Arc<Mutex>）② diarization task 内 `session_id`（258 行）未使用 |
