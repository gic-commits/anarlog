...
async checkPermissions() : Promise<Result<Permissions, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("plugin:shortcut|check_permissions") };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
...
async requestAccessibilityPermission() : Promise<Result<boolean, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("plugin:shortcut|request_accessibility_permission") };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
...
export type Permissions = { accessibility: boolean; inputMonitoring: boolean }
export type ShortcutEvent = { type: "start" } | { type: "stop" } | { type: "cancel" } | { type: "discard" } | { type: "permission"; accessibility: boolean; input_monitoring: boolean }
